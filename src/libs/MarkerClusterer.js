...
// 兼容百度地图JavaScript SDK V3.0 和 GL 版本
var RMap = window.BMap ? window.BMap : window.BMapGL;
var isMapGl = RMap === window.BMapGL;
...
(function() {
    ...
    var getExtendedBounds = function(map, bounds, gridSize) {
        ...
        return new RMap.Bounds(newSW, newNE);
    };
    ...
    var cutBoundsInRange = function(bounds) {
        ...
        return new RMap.Bounds(new RMap.Point(minX, minY), new RMap.Point(maxX, maxY));
    };
    ...
    var MarkerClusterer =
        ...
        (BMapLib.MarkerClusterer = function(map, options) {
            ...
            this._map = map;
            this._mapMaxZoom = isMapGl ? 21 : 19;
            this._markers = [];
            this._clusters = [];
            ...
            this._maxZoom = opts['maxZoom'] || (isMapGl ? 20 : 18);
            ...
            this._maxText = opts['maxText'] || Infinity;
    ...
    function Cluster(markerClusterer) {
        ...
        this._maxText = markerClusterer._maxText;
        this._isHover = this.getIsHover();
        ...
        this._clusterMarker.addEventListener('click', function(event) {
            ...
            if (currentZoom === that._mapMaxZoom) {
                if (that._maxZoomCb && typeof that._maxZoomCb === 'function') {
                    that._maxZoomCb(that._markers, event);
                }
            } else {
                const bound = that.getBounds();
        ...
        if (addHoverEvent) {
            this._clusterMarker.addEventListener('mouseover', function() {
                if (!that.getIsHover()) {
                    that.mouseHoverHandler(true, that, that._clusterMarker);
                }
            });

            this._clusterMarker.addEventListener('mouseout', function() {
                if (that.getIsHover()) {
                    that.mouseHoverHandler(false, that, that._clusterMarker);
                }
            });
        }
    }
     ...
     * 获取鼠标悬浮状态
     */
    Cluster.prototype.getIsHover = function() {
     ...
     * 设置鼠标悬浮状态。
     */
    Cluster.prototype.setIsHover = function(isHover) {
        this._isHover = isHover;
    };

    Cluster.prototype.mouseHoverHandler = function(isHover, cluster, clusterMarker) {
        cluster.setIsHover(isHover);
    ...
    Cluster.prototype.addMarker = function(marker) {
        ...
        if (!this._center) {
            ...
            if (this._isAverageCenter) {
                ...
                this._center = new RMap.Point(lng, lat);
                this.updateGridBounds();
            } //计算新的Center
        }
    ...
    Cluster.prototype.updateGridBounds = function() {
        var bounds = new RMap.Bounds(this._center, this._center);
    ...
    Cluster.prototype.updateClusterMarker = function() {
        ...
        const length = this._markers.length > this._maxText ? `${this._maxText}+` : this._markers.length;
        this._clusterMarker.setText(length);
    };
