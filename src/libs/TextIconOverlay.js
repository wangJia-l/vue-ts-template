...
(function() {
    ...
    T.lang.inherits(TextIconOverlay, RMap.Overlay, 'TextIconOverlay');
    ...
    TextIconOverlay.prototype._updateCss = function() {
        const styles = this.getIsHover() ? this._hoverStyles : this._styles;
        const style = this.getStyleByText(this._text, styles, this._styleInterval);
    ...
    TextIconOverlay.prototype._buildCssText = function(style) {
        ...
        if (T.browser['ie'] < 7) {
            ...
            offset instanceof RMap.Size && (backgroundPosition = offset.width + 'px' + ' ' + offset.height + 'px');
            csstext.push('background-position:' + backgroundPosition + ';');
        }

        if (size instanceof RMap.Size) {
            if (anchor instanceof RMap.Size) {
        ...
        var backgroundRepeat = style['backgroundRepeat'] || 'no-repeat';
        if (backgroundRepeat) {
